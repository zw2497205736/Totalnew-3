...
RangeFinder::RangeFinder(uint32_t inMaxFramesPerSlice, uint32_t inNumFreq, float inStartFreq,
    ...
    float RC = 1.0f / (2.0f * PI * HPF_CUTOFF_FREQ);
    mHPF_alpha = RC / (RC + dt);

    // 初始化WAV写入器
    mWavWriter = new WavWriter();
...
void RangeFinder::RemoveDC() {
    ...
    // ===== 保存去DC后的I/Q信号到WAV =====
    if (mWavWriter && mWavWriter->isRecording()) {
        // 准备交错的立体声数据 (选择第一个频率)
        // L=I路, R=Q路
        float interleavedData[4096 * 2];
        for (uint32_t i = 0; i < mDecsize; i++) {
            interleavedData[i * 2] = mBaseBandReal[0][i];      // L通道
            interleavedData[i * 2 + 1] = mBaseBandImage[0][i]; // R通道
        }
        mWavWriter->writeFloatSamples(interleavedData, mDecsize);
    }
}
...
// ===== WAV录制控制函数 =====
bool RangeFinder::startRecording(const char* filename) {
    if (!mWavWriter) return false;
    
    // 采样率 = 48000 / 16 = 3000 Hz
    // 声道数 = 2 (立体声: L=I, R=Q)
    return mWavWriter->open(filename, 3000, 2);
}

void RangeFinder::stopRecording() {
    if (mWavWriter) {
        mWavWriter->close();
    }
}

bool RangeFinder::isRecording() const {
    return mWavWriter && mWavWriter->isRecording();
}

// ===== 高通滤波器实现 =====
// 一阶IIR高通滤波器：y[n] = alpha * (y[n-1] + x[n] - x[n-1])
// 截止频率：18 kHz
void RangeFinder::ApplyHighPassFilter() {
    // 处理录音缓冲区中的所有样本
    for (uint32_t i = 0; i < mCurRecPos; i++) {
        float x = (float)mRecDataBuffer[i];
        
        // 一阶IIR高通滤波器
        float y = mHPF_alpha * (mHPF_y1 + x - mHPF_x1);
