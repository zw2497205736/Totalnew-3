...
Java_cn_sencs_llap_MainActivity_startRecording(JNIEnv *env, jobject instance, jstring filename_) {
    const char *filename = env->GetStringUTFChars(filename_, 0);
    
    bool result = false;
    if (cd.rangeFinder) {
        result = cd.rangeFinder->startRecording(filename);
    }
...
extern "C"
JNIEXPORT void JNICALL
Java_cn_sencs_llap_MainActivity_stopRecording(JNIEnv *env, jobject instance) {
    if (cd.rangeFinder) {
        cd.rangeFinder->stopRecording();
    }
}

extern "C"
JNIEXPORT jboolean JNICALL
Java_cn_sencs_llap_MainActivity_isRecording(JNIEnv *env, jobject instance) {
    if (cd.rangeFinder) {
        return cd.rangeFinder->isRecording();
    }
    return false;
}
